...
STATIC SMBIOS_TABLE_TYPE0 SmbiosTable0 = {
  ...
  {
    0,
    0x8 /* UefiSpecificationSupported */
  },
  ...
  )
{
  ...
  Status = FdtClient->FindNextCompatibleNode (FdtClient, "baikal,baikal-m", -1, &Node);
  if (EFI_ERROR (Status)) {
    return Status;
  }

  Status = FdtClient->GetNodeProperty (FdtClient, Node, "model", &Prop, NULL);
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  )
{
  ...
  SpdSize = SpdGetSize (Mbm, SpdDdrAddr[Num]);
  if (SpdSize == 0) {
    DEBUG ((
      EFI_D_ERROR,
  ...
  if (SpdSize == 128 || SpdSize == 256) {
    if (SpdSize != SpdGetBuf (Mbm, SpdDdrAddr[Num], Spd, SpdSize)) {
      DEBUG ((
        EFI_D_ERROR,
        "%a: failed to retrieve DDR4 DIMM%d SPD info\n",
        __FUNCTION__,
        Num
        ));
      FreePool (Spd);
    ...
    INTN  Result = SpdGetBuf (Mbm, SpdDdrAddr[Num], Spd, 256);

    if (Result != 256) {
      DEBUG ((
        EFI_D_ERROR,
        "%a: failed to retrieve DDR4 DIMM%d SPD info from Page 0\n",
        __FUNCTION__,
        Num
        ));
      FreePool (Spd);
    ...
    SpdSwitchPage (Mbm, 1);
    Result += SpdGetBuf (Mbm, SpdDdrAddr[Num], Spd + 256, SpdSize - 256);
    SpdSwitchPage (Mbm, 0);
    if (Result != SpdSize) {
      DEBUG ((
        EFI_D_ERROR,
        "%a: failed to retrieve DDR4 DIMM%d SPD info from Page 1\n",
        __FUNCTION__,
        Num
        ));
      FreePool (Spd);
